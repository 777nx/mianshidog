...
基于 Next.js + Spring Cloud + Redis + MySQL + Elasticsearch 构建的程序员智能刷题平台。

致力于程序员刷题，帮助每个人成为编程面试大师🚀
...
3. **安全性**
   ...
   - **缓存热点与热key**
      ...
      ![热点热key](./doc/热点热key.png)
   - **限流与熔断**
   
      ![熔断规则](./doc/熔断规则.png)
   - **IP 黑白名单**
   
      ![IP黑白名单](./doc/IP黑白名单.png)
   - **同端登录冲突检测**
   
      ![同端登录冲突检测](./doc/同端登录冲突检测.png)
   - **分级反爬虫策略**
      - 访问次数超过一定限制，发送邮件
      - 访问次数超过限制太多，封号处理
      
      
      ![分级反爬虫策略](./doc/分级反爬虫策略.png)
